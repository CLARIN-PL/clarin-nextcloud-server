$(document).ready(function() {
	...
	DSpaceConnector.prototype.multipleChoiceFormsData = function(){
		...
		var keyMapping = {
			...
			'size-list': 'metashare. ResourceInfo#TextInfo#SizeInfo. size |         metashare. ResourceInfo#TextInfo#SizeInfo. sizeUnit',
		...
		for(var key in self.lists) {
			if(self.lists.hasOwnProperty(key)) {
				...
				if(key === 'keywords-list' && data === null){
					...
					returnData.push({name: 'dc.subject', value: data});
				}
				else{
					returnData.push({name: keyMapping[key], value: data});
				}
			}
		}
		return returnData;
